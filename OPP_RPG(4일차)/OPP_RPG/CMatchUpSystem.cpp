...
void CMatchUpSystem::applySkill() {
	std::vector<skillData> playerSkill = m_player.getSkill();

	for (int i = 0; i < playerSkill.size(); i++) {
		m_monster.takeDamage(playerSkill[i].m_attackPower);
		m_player.chargeHp(playerSkill[i].m_healPower);
	}
}
